...
def plotit(arg, options):
    ...
    outdir0 = OUTPUTDIR + '/'.join(filepattern.split('/')[:-1]) + '/N%04dS%04d' % (nmax,
                                                                   options.skipfiles)
    outdir = OUTPUTDIR + '/'.join(filepattern.split('/')[:-1]) + '/N%04dW%04dS%04d' % (nmax,
                                                                       options.lmax,
                                                                       options.skipfiles)
    if not os.path.isdir(outdir0):
        subprocess.Popen('mkdir -p %s' % (outdir0 + 'pickles'), shell=True)
        #os.system('mkdir -p %s'%outdir0+"/pickles")
        subprocess.Popen('mkdir -p %s' % (outdir0 + 'gifs'), shell=True)
    ...
    if (os.path.getsize(OUTPUTDIR + filepattern + ".log")) > 100000:
        try:
            subprocess.Popen('tail -1000 %s > tmplog | mv tmplog %s' % (OUTPUTDIR + filepattern + ".log", OUTPUTDIR + filepattern + ".log"))
        except OSError:
            pass
    ...
    if   os.path.isfile(goodcoordsoutfile):
        ...
        print("\n### Need a goodcoords output file. Run the pipeline first!", goodcoordsoutfile)
        sys.exit()
    ...
    pl.show()
    print("\n\n     output file stored in return", bsoutfile.replace("bs.npy", "%d_%d_lcv.pdf" % (x, y)))


if __name__ == '__main__':

    parser = optparse.OptionParser(usage="plotone.py 'filepattern' x y ",
