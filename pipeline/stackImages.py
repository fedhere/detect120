...
#if you have the stack parameters, for example from stcks already processed
...
"""
    ...
    stack = np.zeros_like(stackarray.shape[0])#np.zeros((imsize['nrows'],
            #          imsize['ncols'],
            #          imsize['nbands']))
    if SAVEALL:
...
if __name__ == '__main__':
    ...
    """
        ...
        Uses: 
            findImageSize.findsize():  finds image size by trial and attempt, if not provided or stored in a file.
            also: glob, optparse
       Comments:
