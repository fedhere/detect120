...
def makeGifLcPlot(x, y, ind, ts, xfreq, aperture, fnameroot,
    ...
    if not fft: fig.savefig(fnameroot.replace("gifs","pngs") + "_%d_%d"%(x,y)+".png")
    else: fig.savefig(fnameroot.replace("gifs","pngs") + "_%d_%d"%(x,y)+"_fft.png")
    if showme:
        pl.show()
...
def callplotw(xc, yc, i, ts, xfreq, flist, aperture, imshape, fft=False,
   ...
   if gifs:
       print ([int(yc)-22,int(yc)+21, int(xc)-22,int(xc)+21])
...
#            print ("min and max: ",min(stdbs), max(stdbs))
            ...
            if gifs: makeGifLcPlot(phases['x'][i], phases['y'][i], i,
                                  stdtimeseries, xfreq, 
...
def KMAnalysis(evecs, timeseries, xfreq, allights, stack, img, fnameroot, options):
    ##KMeans cluster analysis. superseded
...
def runit((arg, options)):
    ...
    if options.PCAnalysis:
        ...
        goodphases, phases, tansformed = fit_waves(filepattern, lmax, nmax,
                                                   ...
                                                   fft=options.fft,  gifs=options.gif,
                                                   fold=options.folding,
                                                   mcmc=options.mcmc,
...
if __name__=='__main__':
    ...
    parser.add_option('--nkmc', default=10.0, type="float",
