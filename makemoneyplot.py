...
if __name__ == '__main__':
    ...
    for fi,fl in enumerate([flist1]):#,flist2]):
        ...
        buildingsort = np.argsort(building)
        ...
        if np.isnan(refphases).any(): sys.exit()
        ...
        if options.offset:
            ...
            for i in np.arange(len(phases)):
                ...
                for l in alllights:
                    ...
                    if phasediff[i][l]>1: phasediff[i][l]=2.0-phasediff[i][l]
                    if phasediff[i][l]<-1: phasediff[i][l]=2.0+phasediff[i][l]
                    if phasediff[i][l]<0: phasediff[i][l]=2.0+phasediff[i][l]
                    ...
                    if i>0:
                        if np.abs(phasediff[i-1][l]-mymode[i-1] -\
                            ...
                            ax.plot([i-1,i]+offsetsx[l],
                                    [(phasediff[i-1][l]-mymode[i-1]) + offsets[l],
                                     (phasediff[i][l]-mymode[i]) + offsets[l]],
                                    '-', color = mycm[l], alpha=0.8, lw=1.8)
                            #phasediff_old[l] = phasediff[l]

                        if np.abs(phasediff[i-1][l]-myref[i-1] - \
                                  phasediff[i][l]+myref[i])<0.5:
                            #print (i, phasediff_old)
    ...
    if not options.offset and TWODPLOT:
            ...
            for i in np.arange(len(phases)):                 
                ax.scatter([i]*len(phasediff[i])+offsetsx,
                           (phasediff[i]-mymode[i]) + offsets,
                            color = mycm, alpha=0.5, s=8, edgecolor='k')
        ...
        ax.set_ylabel("corrected phase %s (radians)"%(fft[fi]), fontsize=13)
        ...
        axref.set_ylabel("corrected phase %s (radians)"%(fft[fi]), fontsize=13)
        if not options.onerun:
            ...
            axref.set_xticklabels(['%s'%tm for tm in np.arange(len(flist1))*15])
        else:
            ax.set_xlabel("time (seconds from first run)", fontsize=13)
            ax.set_xticks(range(len(flist1)))
            ax.set_xticklabels(['%s'%tm for tm in np.arange(len(flist1))*25])
            axref.set_xticks(range(len(flist1)))
            ax.set_xticklabels(['%s'%tm for tm in np.arange(len(flist1))*25])            
...
#        pl.xticks(range(len(flist1)),['%s'%tm for tm in np.arange(len(flist1))*15])
    ...
    if TWODPLOT:
        if not options.onerun:
            ...
            if options.offset:
                pl.savefig("moneyplot_offset_%s_%d_%d.pdf"%(options.onerun,
                                                        ref[0], ref[1]))
            else:
                pl.savefig("moneyplot_phases_%s_%d_%d.pdf"%(options.onerun,
                                                        ref[0], ref[1]))
    #pl.show()
    #pl.close('all')
    ...
    fig, ((ax[0],ax[1]),(ax[2],ax[3])) = pl.subplots(2, 2, sharex='col', sharey='row')
    def distance(a,b):
        if np.isnan(a*b):
            return np.nan
        return np.abs(a-b)
    ...
    if not options.onerun:
        if options.offset:
            pl.savefig("phasematrix_offset.pdf")
        else:
            pl.savefig("phasematrix_phases.pdf")
    else:
        if options.offset:
            pl.savefig("phasematrix_offset_%s.pdf"%(options.onerun))
        else:
            pl.savefig("phasematrix_phases_%s.pdf"%(options.onerun))
    pl.close('all')
